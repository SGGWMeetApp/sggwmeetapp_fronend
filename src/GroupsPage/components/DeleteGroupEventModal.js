...
const GROUP_EVENTS_URL = 'http://3.68.195.28/api/groups';

const DeleteGroupEventModal = ({ setDisplayDeleteModal, groupId }) => {
    const { eventId } = useEventsContext();
    const { user } = useAuthContext();

    const handleDelete = async () => {
        const response = await fetch(`${GROUP_EVENTS_URL}/${groupId}/events/${eventId}`, {
            method: 'DELETE',
...
export default DeleteGroupEventModal;